...
def get_stop_line(roi_frame):
    ...
    polygon = np.array([[(0, height), (0, height // 2 + 50), (width, height // 2 + 50), (width, height)]])
...
def detect_and_draw(roi_frame, model, y_stop):
    ...
    for box in boxes:
        ...
        if class_names[cls_id] == 'red_light':
            ...
            cv2.putText(roi_frame, "Red Light", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)
            break
...
def main():
    ...
    # Khởi tạo parser để đọc đối số dòng lệnh
    parser = argparse.ArgumentParser()
    parser.add_argument('--source', type=str, required=True, help='Input video file path')
    parser.add_argument('--weights', type=str, default='my_model.pt', help='YOLO model weights path')
    ...
    # Cấu hình VideoWriter để lưu thành file mp4 thay vì hiển thị
    ...
    # Giải phóng tài nguyên bộ nhớ sau khi hoàn thành
    cap.release()
    out.release()
